...
public class DomainExceptionHandlerTests
{
    ...
    {
        Microsoft.AspNetCore.Http.ProblemDetailsContext ctx = Context();
        TestValidationException exception = new([new ValidationFailure("Name", "'Name' is required.")]);
    ...
    public void Handle_Validation_MergesPropertiesThatCollideUnderThePolicy()
    {
        ...
        Microsoft.AspNetCore.Http.ProblemDetailsContext ctx = Context();
        TestValidationException exception = new([new ValidationFailure("Id", "first"), new ValidationFailure("ID", "second")]);
